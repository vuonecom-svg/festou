...
function clausulasDo(termos: Termos): [string, string][] {
  const itens = termos.itens.toLowerCase();
  const infl: [string, string][] = termos.inflavel
    ? [
    ...
    : [["Uso correto e conservação", `O LOCATÁRIO é responsável pelo uso correto e pela conservação dos ${itens} e itens contratados durante todo o período do evento.`]];

  return [
    ["Objeto", `A LOCADORA cede em locação ao LOCATÁRIO os ${itens} e itens discriminados neste contrato, pelo período e local do evento aqui indicados.`],
    ...
    ["Danos e responsabilidade", "O LOCATÁRIO responde por perdas ou danos que excedam o desgaste normal de uso, decorrentes de mau uso, vandalismo ou descuido, arcando com o custo de reparo ou reposição."],
    ...
    ["Cancelamento e reagendamento", "Cancelamentos com menos de 48 horas de antecedência não dão direito à devolução do sinal. O reagendamento está sujeito à disponibilidade de data e dos itens contratados."],
    ["Multa por atraso", "A retenção além do horário combinado, por responsabilidade do LOCATÁRIO, poderá gerar cobrança adicional proporcional à diária."],
  ...
  return (
    <Document title={`Contrato ${p.numero}`}>
      <Page size="A4" style={s.page}>
        ...
        <Text style={s.title}>{termos.contratoTitulo}</Text>
        ...
        <Text style={s.h}>{termos.itens} e itens locados</Text>
        <View style={s.tHead}>
        ...
        {clausulasDo(termos).map(([titulo, texto], i) => (
        ...
        <Text style={s.footer}>Gerado por FesFlow — gestão para empresas de festa.</Text>
      </Page>
    </Document>
  );
}
