...
export type PedidoStatusFin = "aguardando_sinal" | "sinal_pago" | "quitado" | "reembolsado";
...
// Mapas cobrem TODOS os valores do enum do banco (não só os que a UI grava) —
// um UPDATE de suporte/import com valor "extra" não pode derrubar a lista.
export const PEDIDO_FIN: Record<string, { label: string; badge: string }> = {
  ...
  reembolsado: { label: "Reembolsado", badge: "bg-rose-100 text-rose-700" },
};
...
export const PEDIDO_OP: Record<string, { label: string; badge: string }> = {
  ...
  separado: { label: "Separado", badge: "bg-slate-100 text-slate-700" },
  carregado: { label: "Carregado", badge: "bg-indigo-100 text-indigo-700" },
  ...
  em_montagem: { label: "Em montagem", badge: "bg-teal-100 text-teal-700" },
  montado: { label: "Montado", badge: "bg-teal-100 text-teal-700" },
  em_evento: { label: "Em evento", badge: "bg-blue-100 text-blue-700" },
  em_retirada: { label: "Em retirada", badge: "bg-violet-100 text-violet-700" },
  ...
  reagendado: { label: "Reagendado", badge: "bg-amber-100 text-amber-700" },
};

// Fallback à prova de valor desconhecido (defesa em profundidade nos renders).
export function statusInfo(
  mapa: Record<string, { label: string; badge: string }>,
  chave: string
): { label: string; badge: string } {
  return mapa[chave] ?? { label: chave, badge: "bg-slate-100 text-slate-700" };
}
